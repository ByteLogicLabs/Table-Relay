//! `Adapter::browse` + `count_records` for the SQLite adapter. Modeled
//! on the MySQL implementation but with SQLite's double-quoted
//! identifiers and without a schema-qualified `FROM`.

use std::time::Instant;

use adapter_api::log_line;
use adapter_api::{
    AdapterError, BrowseRequest, BrowseResult, ColumnMeta, CountRequest, Filter, FilterOp,
};
use serde_json::Value as JsonValue;
use sqlx::sqlite::{SqliteArguments, SqliteColumn, SqliteRow};
use sqlx::query::Query;
use sqlx::{Column, Row, Sqlite, TypeInfo};

use crate::SqliteDriver;
use crate::sqlite::{bind_json, column_to_json, quote_ident};

pub(crate) async fn browse(
    driver: &SqliteDriver,
    req: BrowseRequest,
) -> Result<BrowseResult, AdapterError> {
    ensure_main(&req.schema)?;
    let t_total = Instant::now();

    let page_number = req.page.number.max(1);
    let page_size = req.page.size.max(1);
    let offset = (page_number as u64 - 1) * (page_size as u64);

    let mut sql = String::from("SELECT * FROM ");
    sql.push_str(&quote_ident(&req.table));

    let (where_sql, filter_values) = build_where(&req.filters)?;
    sql.push_str(&where_sql);

    if !req.sort.is_empty() {
        sql.push_str(" ORDER BY ");
        for (i, s) in req.sort.iter().enumerate() {
            if i > 0 {
                sql.push_str(", ");
            }
            sql.push_str(&quote_ident(&s.column));
            sql.push(' ');
            sql.push_str(match s.direction {
                adapter_api::SortDirection::Asc => "ASC",
                adapter_api::SortDirection::Desc => "DESC",
            });
        }
    }

    sql.push_str(&format!(" LIMIT {page_size} OFFSET {offset}"));

    log_line!(
        "browse",
        "{}.{} page={} size={} filters={} sort={} offset={}",
        req.schema,
        req.table,
        page_number,
        page_size,
        req.filters.len(),
        req.sort.len(),
        offset,
    );

    let mut q: Query<'_, Sqlite, SqliteArguments> = sqlx::query(&sql);
    for v in &filter_values {
        q = bind_json(q, v);
    }

    let count_fut = async {
        if req.include_total {
            count_inner(driver, &req.table, &req.filters).await
        } else {
            Ok(None)
        }
    };

    let rows_fut = async { q.fetch_all(&driver.pool).await.map_err(AdapterError::from) };

    // Declared column types from the schema. sqlx's SQLite driver only
    // surfaces each result column's *affinity* via `type_info().name()`
    // (INTEGER / REAL / TEXT / BLOB / NUMERIC), which loses the user's
    // declared type — a column declared `BOOLEAN` or `DATETIME` comes
    // back as `NUMERIC` and the UI's type-based editors (checkbox,
    // calendar) misfire. `PRAGMA table_info` gives us the declared type
    // string verbatim, which is what we want to hand the frontend.
    let declared_types_fut = fetch_declared_types(driver, &req.table);

    let (rows_res, count_res, declared_res) =
        tokio::join!(rows_fut, count_fut, declared_types_fut);
    let rows: Vec<SqliteRow> = rows_res?;
    let total_records = count_res?;
    // A declared-types failure is non-fatal — we log it and fall back to
    // the affinity-based type_hint so the grid still renders.
    let declared: std::collections::HashMap<String, String> =
        declared_res.unwrap_or_default();

    let columns: Vec<ColumnMeta> = if let Some(first) = rows.first() {
        first
            .columns()
            .iter()
            .map(|c: &SqliteColumn| {
                let name = c.name().to_string();
                let hint = declared
                    .get(&name)
                    .cloned()
                    .unwrap_or_else(|| c.type_info().name().to_string());
                ColumnMeta { name, type_hint: hint }
            })
            .collect()
    } else {
        let mut conn = driver.pool.acquire().await?;
        use sqlx::Executor;
        conn.describe(sql.as_str())
            .await
            .ok()
            .map(|d| {
                d.columns()
                    .iter()
                    .map(|c| {
                        let name = c.name().to_string();
                        let hint = declared
                            .get(&name)
                            .cloned()
                            .unwrap_or_else(|| c.type_info().name().to_string());
                        ColumnMeta { name, type_hint: hint }
                    })
                    .collect()
            })
            .unwrap_or_default()
    };

    // type_names feeds `column_to_json` for per-cell decoding. The
    // affinity — not the declared type — is what drives decoder choice
    // because that's what SQLite actually hands us over the wire; the
    // declared type only affects how the UI renders.
    let type_names: Vec<String> = if let Some(first) = rows.first() {
        first
            .columns()
            .iter()
            .map(|c| c.type_info().name().to_string())
            .collect()
    } else {
        columns.iter().map(|c| c.type_hint.clone()).collect()
    };

    let json_rows: Vec<Vec<JsonValue>> = rows
        .iter()
        .map(|row| {
            row.columns()
                .iter()
                .enumerate()
                .map(|(i, _)| {
                    column_to_json(row, i, type_names.get(i).map(|s| s.as_str()).unwrap_or(""))
                })
                .collect()
        })
        .collect();

    let duration_ms = t_total.elapsed().as_secs_f64() * 1000.0;
    log_line!(
        "browse",
        "  → {} rows ({:.1}ms), total_records={:?}",
        json_rows.len(),
        duration_ms,
        total_records,
    );

    Ok(BrowseResult {
        columns,
        rows: json_rows,
        duration_ms,
        page: page_number,
        total_records,
    })
}

/// Read declared column types from `PRAGMA table_info(<table>)`. The
/// map is keyed by column name; values are the user's declared type
/// string (e.g. `"INTEGER"`, `"VARCHAR(255)"`, `"DATETIME"`).
///
/// Returns an empty map when the table isn't known (expression queries,
/// views without a directly inspectable source). Callers treat that as
/// "no overrides — use the affinity".
async fn fetch_declared_types(
    driver: &SqliteDriver,
    table: &str,
) -> Result<std::collections::HashMap<String, String>, AdapterError> {
    let rows: Vec<(i64, String, String, i64, Option<String>, i64)> =
        sqlx::query_as(&format!("PRAGMA table_info({})", quote_ident(table)))
            .fetch_all(&driver.pool)
            .await?;
    Ok(rows
        .into_iter()
        .filter(|r| !r.2.is_empty()) // skip columns with no declared type
        .map(|(_cid, name, decl_type, _notnull, _default, _pk)| (name, decl_type))
        .collect())
}

pub(crate) async fn count_records(
    driver: &SqliteDriver,
    req: CountRequest,
) -> Result<Option<u64>, AdapterError> {
    ensure_main(&req.schema)?;
    count_inner(driver, &req.table, &req.filters).await
}

async fn count_inner(
    driver: &SqliteDriver,
    table: &str,
    filters: &[Filter],
) -> Result<Option<u64>, AdapterError> {
    let mut sql = String::from("SELECT COUNT(*) FROM ");
    sql.push_str(&quote_ident(table));
    let (where_sql, filter_values) = build_where(filters)?;
    sql.push_str(&where_sql);

    let mut q = sqlx::query_scalar::<_, i64>(&sql);
    for v in &filter_values {
        q = match v {
            JsonValue::Null => q.bind(Option::<String>::None),
            JsonValue::Bool(b) => q.bind(*b),
            JsonValue::Number(n) => {
                if let Some(i) = n.as_i64() {
                    q.bind(i)
                } else if let Some(f) = n.as_f64() {
                    q.bind(f)
                } else {
                    q.bind(n.to_string())
                }
            }
            JsonValue::String(s) => q.bind(s.clone()),
            JsonValue::Array(_) | JsonValue::Object(_) => {
                q.bind(serde_json::to_string(v).unwrap_or_default())
            }
        };
    }

    let n: i64 = q.fetch_one(&driver.pool).await?;
    Ok(Some(n.max(0) as u64))
}

fn ensure_main(schema: &str) -> Result<(), AdapterError> {
    if schema.is_empty() || schema.eq_ignore_ascii_case("main") {
        Ok(())
    } else {
        Err(AdapterError::NotFound(format!(
            "SQLite adapter only exposes the `main` schema; got `{schema}`"
        )))
    }
}

fn build_where(filters: &[Filter]) -> Result<(String, Vec<JsonValue>), AdapterError> {
    if filters.is_empty() {
        return Ok((String::new(), Vec::new()));
    }
    let mut sql = String::from(" WHERE ");
    let mut values: Vec<JsonValue> = Vec::new();
    for (i, f) in filters.iter().enumerate() {
        if i > 0 {
            sql.push_str(" AND ");
        }
        sql.push_str(&quote_ident(&f.column));
        match f.op {
            FilterOp::Eq => {
                sql.push_str(" = ?");
                values.push(require_value(f, "eq")?);
            }
            FilterOp::NotEq => {
                sql.push_str(" <> ?");
                values.push(require_value(f, "not_eq")?);
            }
            FilterOp::Lt => {
                sql.push_str(" < ?");
                values.push(require_value(f, "lt")?);
            }
            FilterOp::Lte => {
                sql.push_str(" <= ?");
                values.push(require_value(f, "lte")?);
            }
            FilterOp::Gt => {
                sql.push_str(" > ?");
                values.push(require_value(f, "gt")?);
            }
            FilterOp::Gte => {
                sql.push_str(" >= ?");
                values.push(require_value(f, "gte")?);
            }
            FilterOp::Contains => {
                sql.push_str(" LIKE ?");
                values.push(like_wrap(f, "%{}%")?);
            }
            FilterOp::NotContains => {
                sql.push_str(" NOT LIKE ?");
                values.push(like_wrap(f, "%{}%")?);
            }
            FilterOp::StartsWith => {
                sql.push_str(" LIKE ?");
                values.push(like_wrap(f, "{}%")?);
            }
            FilterOp::EndsWith => {
                sql.push_str(" LIKE ?");
                values.push(like_wrap(f, "%{}")?);
            }
            FilterOp::IsNull => {
                sql.push_str(" IS NULL");
            }
            FilterOp::IsNotNull => {
                sql.push_str(" IS NOT NULL");
            }
            FilterOp::In | FilterOp::NotIn => {
                let list = f.value.as_ref().and_then(|v| v.as_array()).ok_or_else(|| {
                    AdapterError::Unsupported(format!("{:?} requires an array value", f.op))
                })?;
                if list.is_empty() {
                    sql.push_str(match f.op {
                        FilterOp::In => " IN (NULL) AND 1=0",
                        _ => " IS NOT NULL",
                    });
                } else {
                    sql.push_str(match f.op {
                        FilterOp::In => " IN (",
                        _ => " NOT IN (",
                    });
                    for (j, item) in list.iter().enumerate() {
                        if j > 0 {
                            sql.push_str(", ");
                        }
                        sql.push('?');
                        values.push(item.clone());
                    }
                    sql.push(')');
                }
            }
        }
    }
    Ok((sql, values))
}

fn require_value(f: &Filter, op_name: &str) -> Result<JsonValue, AdapterError> {
    f.value.clone().ok_or_else(|| {
        AdapterError::Unsupported(format!("`{op_name}` filter on `{}` has no value", f.column))
    })
}

fn like_wrap(f: &Filter, template: &str) -> Result<JsonValue, AdapterError> {
    let v = require_value(f, "like")?;
    let s = match v {
        JsonValue::String(s) => s,
        JsonValue::Null => {
            return Err(AdapterError::Unsupported(format!(
                "null value not supported for LIKE filter on `{}`",
                f.column
            )));
        }
        JsonValue::Bool(b) => b.to_string(),
        JsonValue::Number(n) => n.to_string(),
        _ => v.to_string(),
    };
    Ok(JsonValue::String(template.replace("{}", &s)))
}

#[cfg(test)]
mod tests {
    use super::*;
    use serde_json::json;

    fn filter(col: &str, op: FilterOp, value: Option<JsonValue>) -> Filter {
        Filter {
            column: col.to_string(),
            op,
            value,
        }
    }

    #[test]
    fn eq_uses_double_quoted_ident() {
        let (sql, _) = build_where(&[filter("id", FilterOp::Eq, Some(json!(1)))]).unwrap();
        assert_eq!(sql, " WHERE \"id\" = ?");
    }

    #[test]
    fn empty_in_short_circuits() {
        let (sql, values) =
            build_where(&[filter("status", FilterOp::In, Some(json!([])))]).unwrap();
        assert!(sql.contains("1=0"), "got {sql}");
        assert!(values.is_empty());
    }

    #[test]
    fn double_quote_in_name_escaped() {
        let (sql, _) = build_where(&[filter("weird\"col", FilterOp::Eq, Some(json!(1)))]).unwrap();
        assert_eq!(sql, " WHERE \"weird\"\"col\" = ?");
    }
}
