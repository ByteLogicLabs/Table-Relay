...
interface SidebarProps {
  ...
  onOpenStructure: (connectionId: string, schema: string, tableName: string) => void;
  onNewQuery: (connectionId: string, tableName?: string) => void;
  onOpenErd: (connectionId: string, schemaName: string, tableName?: string) => void;
   ...
   */
  onOpenDefinition: (connectionId: string, key: string, title: string, sql: string) => void;
  /** Open a dedicated routine editor tab for a function/procedure. */
  onOpenRoutine?: (connectionId: string, schema: string, name: string, kind: 'function' | 'procedure') => void;
  ...
  onNewRoutine?: (connectionId: string, schema: string, kind: 'function' | 'procedure') => void;
...
export default function Sidebar({
  ...
  const [viewsByDb, setViewsByDb] = useState<Map<string, ViewInfo[]>>(new Map());
  const [routinesByDb, setRoutinesByDb] = useState<Map<string, RoutineInfo[]>>(new Map());
  ...
  const isLoadingSchemas = conn ? connState.loadingSchemasById.has(conn.id) : false;
  ...
  useEffect(() => {
    ...
    void Promise.all([viewsFut, routinesFut])
      ...
      .finally(() => setLoadingExtras(false));
  }, [conn, effectiveSchema, viewsByDb, routinesByDb, supportsViews, supportsRoutines]);
  ...
  const fTables = q ? tables.filter(t => t.name.toLowerCase().includes(q)) : tables;
  ...
  const matchesActive = (
    ...
    if (type === 'routine' && routineKind && activeItem.routineKind !== routineKind) return false;
    return true;
  };
  ...
  const pickDdlColumn = (columns: { name: string }[], label: string): number => {
    const target = label.toLowerCase();
    return columns.findIndex(c => c.name.toLowerCase() === target);
  };

  const openViewDefinition = async (connectionId: string, dbName: string, viewName: string) => {
    try {
      // No row limit — `SHOW CREATE` rejects a trailing `LIMIT`.
      const res = await db.runQuery(connectionId, `SHOW CREATE VIEW \`${dbName}\`.\`${viewName}\``);
      ...
      onOpenDefinition(connectionId, `view:${dbName}.${viewName}`, viewName, `USE \`${dbName}\`;\n\n${editable};\n`);
    } catch (err) {
      toast.error(`Could not load view: ${err instanceof Error ? err.message : String(err)}`);
    }
  };
  ...
  const openRoutineDefinition = async (
    ...
    try {
      const res = await db.runQuery(connectionId, `SHOW CREATE ${k} \`${dbName}\`.\`${routineName}\``);
      const last = res.statements[res.statements.length - 1];
      if (!last || last.error) throw new Error(last?.error ?? 'no definition returned');
      const idx = pickDdlColumn(last.columns, `Create ${k.charAt(0) + k.slice(1).toLowerCase()}`);
      const ddl = idx >= 0 ? last.rows[0]?.[idx] : null;
      if (typeof ddl !== 'string') throw new Error('definition not found in response');
      ...
      onOpenDefinition(connectionId, `${k.toLowerCase()}:${dbName}.${routineName}`, routineName, editable);
    } catch (err) {
      toast.error(`Could not load ${k.toLowerCase()}: ${err instanceof Error ? err.message : String(err)}`);
    }
  };
      ...
      <div className="px-3 pt-3 pb-2 border-b border-border/50">
        <div className="group flex items-center gap-2 mb-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[10px] text-muted-foreground truncate">{conn.driver} · {conn.host}</div>
          </div>
          <DropdownMenu>
            <DropdownMenuTrigger
              render={(props) => (
                <Button {...props} variant="ghost" size="icon" className="h-6 w-6 shrink-0 opacity-0 group-hover:opacity-100 focus:opacity-100 data-popup-open:opacity-100">
            ...
            <DropdownMenuContent align="end" className="min-w-48">
              ...
              {selectedDb && supportsDiagram && (
                <DropdownMenuItem onClick={() => onOpenErd(conn.id, schemaForActions)}>
        ...
        <div className="py-2 space-y-1 px-1">
          ...
          {conn && !isConnected && !isConnecting && connectError && (
            <div className="px-4 py-6 flex flex-col items-center gap-3 text-center">
              ...
              <div className="text-xs font-medium">Couldn't connect to {conn.name}</div>
          ...
          {/* Post-connect schema fetch. `connectAndLoad` fires
              ...
              the schemas Map still has nothing for this connection —
              the latter covers the tiny window between "connected" and
              "refreshSchemas started" (and a silent-failure fallback).
          ...
          {isConnected && !selectedDb && !isLoadingSchemas && schemas.length > 0 && !showLoadingFloor && (
            <div className="px-4 py-6 flex flex-col items-center gap-2 text-center text-xs text-muted-foreground">
              ...
              <Button size="sm" variant="outline" onClick={() => setDbPickerOpen(true)}>
          ...
          {isConnected && selectedDb && schemas.length > 0 && !isLoadingSchemas && !showLoadingFloor && (
            <>
              ...
              {!collapsed.tables && (
                ...
                <ContextMenu key={`t-${t.name}`}>
                  ...
                  <ContextMenuContent className="w-52">
                    ...
                    <ContextMenuItem onClick={() => onOpenStructure(conn.id, schemaForActions, t.name)}>
                      Open Schema
                    </ContextMenuItem>
                    <ContextMenuItem onClick={() => onNewQuery(conn.id, t.name)}>
                      New query
                    </ContextMenuItem>
                    {supportsDiagram && (
                      <>
                        <ContextMenuSeparator />
                        <ContextMenuItem onClick={() => onOpenErd(conn.id, schemaForActions, t.name)}>
                          View ER diagram
                        </ContextMenuItem>
                      </>
                    )}
              ...
              {supportsViews && <Section
                ...
                onAdd={onNewView ? () => onNewView(conn.id, schemaForActions) : undefined}
                addTitle="New view"
              />}
              {supportsViews && !collapsed.views && fViews.map(v => {
                ...
                <ContextMenu key={`v-${v.name}`}>
                  <ContextMenuTrigger>
                    <button
                      ...
                      onClick={() => void openViewDefinition(conn.id, schemaForActions, v.name)}
                    >
                      <LayoutTemplate className={iconCls(active)} />
                      <span className="flex-1 truncate">{v.name}</span>
                      {v.isUpdatable && <span className="text-[9px] text-muted-foreground/70">upd</span>}
                    </button>
                  </ContextMenuTrigger>
                  <ContextMenuContent className="w-48">
                    <ContextMenuItem onClick={() => onOpenTable(conn.id, schemaForActions, v.name)}>
                      Open data
                    </ContextMenuItem>
                    <ContextMenuItem onClick={() => void openViewDefinition(conn.id, schemaForActions, v.name)}>
                      Edit definition
                    </ContextMenuItem>
                    <ContextMenuItem onClick={() => onOpenStructure(conn.id, schemaForActions, v.name)}>
                      Open Schema
                    </ContextMenuItem>
                  </ContextMenuContent>
                </ContextMenu>
              ...
              {supportsRoutines && <Section
                label="routines"
                count={fRoutines.length}
                loading={loadingExtras && routines.length === 0}
                collapsed={collapsed.routines}
                onToggle={() => toggle('routines')}
                onAdd={onNewRoutine ? () => onNewRoutine(conn.id, schemaForActions, 'function') : undefined}
                addTitle="New function"
              />}
              {supportsRoutines && !collapsed.routines && fRoutines.map(r => {
                const sig = r.parameters
                  .map(p => `${p.mode ?? 'IN'} ${p.name}:${p.dataType}`)
                  .join(', ');
                const ret = r.returns ? ` → ${r.returns}` : '';
                const tooltip = `${r.kind}(${sig})${ret}`;
                const routineKind = (r.kind.toLowerCase() === 'function' ? 'function' : 'procedure') as 'function' | 'procedure';
                const active = matchesActive('routine', r.name, routineKind);
                return (
                  <ContextMenu key={`r-${r.name}`}>
                    <ContextMenuTrigger>
                      <button
                        ...
                        title={tooltip}
                        onClick={() => {
                          if (onOpenRoutine) onOpenRoutine(conn.id, schemaForActions, r.name, routineKind);
                          else void openRoutineDefinition(conn.id, schemaForActions, r.name, r.kind);
                        }}
                      >
                        <FunctionSquare className={iconCls(active)} />
                        <span className="flex-1 truncate">{r.name}</span>
                        <span className={`text-[9px] ${active ? 'text-primary/70' : 'text-muted-foreground/70'}`}>{r.kind}</span>
                      </button>
                    </ContextMenuTrigger>
                    <ContextMenuContent className="w-48">
                      <ContextMenuItem
                        onClick={() => {
                          const k = (r.kind.toLowerCase() === 'function' ? 'function' : 'procedure') as 'function' | 'procedure';
                          if (onOpenRoutine) onOpenRoutine(conn.id, schemaForActions, r.name, k);
                          else void openRoutineDefinition(conn.id, schemaForActions, r.name, r.kind);
                        }}
                      >
                        Edit definition
                      ...
                      <ContextMenuItem
                        onClick={() => void openRoutineDefinition(conn.id, schemaForActions, r.name, r.kind)}
                      >
                        Edit as SQL
                      </ContextMenuItem>
                      <ContextMenuItem
                        onClick={() => {
                          const call = r.kind === 'function'
                      ...
                      <ContextMenuItem onClick={() => { void navigator.clipboard.writeText(r.name); toast.success('Name copied'); }}>
                        Copy name
                      </ContextMenuItem>
                    </ContextMenuContent>
                  </ContextMenu>
              ...
              {fTables.length === 0 && fViews.length === 0 && fRoutines.length === 0 && (
